...
class EndsWithEngine implements MatchingEngine {
	 ...
	 */
	protected EndsWithEngine(final char[] lowerCase, final char[] upperCase, final boolean[] matchOne, final int length) {
	...
	public boolean matches(final String string) {
		...
		for (int patternIndex = length - 1; patternIndex > 0; --patternIndex) {
			if (!matchOne[patternIndex] && string.charAt(charsIndex) != lowerCase[patternIndex] && string.charAt(charsIndex) != upperCase[patternIndex]) {
				return false;
			}
			charsIndex--;
		}
